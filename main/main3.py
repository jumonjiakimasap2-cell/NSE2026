...
フェーズ構成:
    ...
    Phase 4 : GPS 誘導走行 (目標地点 PHASE4_TO_5_RADIUS = 2 m 以内まで)
                ...
                まとめてログ表示する。
                【改良】機体が目標のほぼ真逆 (方位差 |diff| が180°付近) を
                向いている場合、diff の符号がセンサノイズで +180/-180 の
                境界を頻繁に行き来し、左旋回・右旋回の指令が交互に入れ替
                わって拮抗し、目標と反対方向へ流れてしまう不具合があった。
                これに対応するため、|diff| が ANGLE_180_PREFER_RIGHT_DEG
                (170°) 以上のときは diff の符号によらず必ず右回転を選ぶ
                不感帯を設け (apply_diff)、旋回方向を一意に固定して
                拮抗を防ぐようにした。
...
def calc_azimuth(mag: list) -> float:
    """
    生の地磁気ベクトルから真方位角 [度, 北=0, 東=90, 南=180, 西=270,
    時計回り] を計算する。
    mag[0]: 機体前方軸の地磁気成分
    mag[1]: 機体右方向軸の地磁気成分

    ★修正 (重大バグ): 従来の式 `90.0 - atan2(mag[1], mag[0])` の後に
    `*= -1` を掛けるロジックを展開すると、
        azimuth = -90 + atan2(mag[1], mag[0]) + MAG_DECLINATION
    となる。これを数値検証すると、真方位 0°(北)/90°(東)/180°(南)/270°(西)
    に対し、計算結果は 262°/172°/82°/352° という「定数262° から真方位を
    引いた値」になっており、単なる符号や定数のズレではなく、真方位に
    対して鏡写し (東西が反転する reflection) になっていることが判明した。
    そのため方位差 (diff) に基づく旋回制御が正しく収束せず、目標の
    真逆へ向かってしまう不具合が起きていた。

    正しくは、機体前方軸 (mag[0]) ・右方向軸 (mag[1]) の地磁気成分から
    `atan2(-mag[1], mag[0])` で北基準・時計回りの方位角が得られる
    (mag[1] の符号を反転させるのがポイントで、単純な `az *= -1` の
    ような「計算結果全体の符号反転」とは異なる)。この式であれば
    北→0°, 東→90°, 南→180°, 西→270° と真方位に正しく一致することを
    数値検証済み。

    ※ もし実機でセンサの Y 軸 (mag[1]) が「左方向」を指す取り付けに
    なっている場合は、符号を反転させず `atan2(mag[1], mag[0])` に
    変更すること (BNO_ORIENTATION の Axis Remap 設定には手を加えない)。
    """
    azimuth_mag   = math.degrees(math.atan2(-mag[1], mag[0]))
    true_azimuth  = azimuth_mag + MAG_DECLINATION
    return true_azimuth % 360.0
