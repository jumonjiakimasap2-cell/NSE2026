...
# ===========================================================================
# GPS スレッド (test_finishv.py の gps_thread_func と同じ構造)
# ===========================================================================
        ...
        with serial.Serial(port, baudrate, timeout=1.0) as ser:
            ...
            while True:
                try:
                    ...
                    if gps_obj.valid:
                        lat_raw = gps_obj.latitude   # [deg, decimal_min, 'N'/'S']
                        lng_raw = gps_obj.longitude  # [deg, decimal_min, 'E'/'W']

                        # ddm → dd 変換 (test_finishv.py と同じ手順)
                        lat = lat_raw[0] + lat_raw[1] / 60.0
                        if lat_raw[2] == 'S':
                            lat = -lat
                        lng = lng_raw[0] + lng_raw[1] / 60.0
                        if lng_raw[2] == 'W':
                            lng = -lng
...
def calc_distance(lat: float, lng: float) -> float:
    ...
    NICS2026/main.py calcdistance() と同じ計算式。
    """
...
def calc_target_bearing(lat: float, lng: float) -> float:
    ...
    NICS2026/main.py calcAngle() と同じ計算式。
    """
    dx = math.radians(TARGET_LNG - lng) * EARTH_RADIUS * math.cos(math.radians(lat))
    dy = math.radians(TARGET_LAT - lat) * EARTH_RADIUS
...
def calc_azimuth(mag: list) -> float:
    ...
    NICS2026/main.py calcAzimuth() と同じ計算式に
    地磁気偏角補正 (MAG_DECLINATION) を加えた。
    """
...
def calc_direction_diff(azimuth: float, target_bearing: float) -> float:
    ...
    NICS2026/p3_run.py の diff 計算と同じロジック。
    """
...
# --- ピン (BCM) ── test_run.py / fall.py と同じ ---
PIN_PWMA = 13
PIN_AIN1 =  5
PIN_AIN2 =  6
PIN_PWMB = 18
...
class MotorController:
    ...
    モータ配置:
        motor_a (PWMA, AIN1, AIN2) = 右モータ
        motor_b (PWMB, BIN1, BIN2) = 左モータ
    """
    ...
    def apply_diff(self, diff: float) -> str:
        """
        diff (−180〜+180) からモータコマンドを自動選択する。
        NICS2026/p3_run.py の motor 指令分岐と同じ判定順序。
...
def main():
    ...
    # ── BNO055 初期化 (fall.py / test_finishv.py と同じ) ──
    print("\n[INIT] BNO055 初期化中...")
    bno = BNO055()
    ...
    # ── GPS スレッド起動 (test_finishv.py と同じ) ──
    ...
    try:
        while True:
            ...
            # ── GPS 未取得なら停止して待機 (NICS2026/p3_run.py の GPS待機と同じ考え) ──
            if not gps_valid:
                motor.stop()
                motor_cmd = "GPS_WAIT"
            else:
