...
# ===========================================================================
# 航法計算 (緯度・経度座標に基づく正確な球面方位角・距離計算)
# ===========================================================================

def calc_distance(lat: float, lng: float, target_lat: float = TARGET_LAT, target_lng: float = TARGET_LNG) -> float:
    """
    現在地 (lat, lng) から目標座標 (target_lat, target_lng) までの
    大円距離 [m] を返す (Haversine の公式)。
    """
    phi1 = math.radians(lat)
    phi2 = math.radians(target_lat)
    delta_phi = math.radians(target_lat - lat)
    delta_lambda = math.radians(target_lng - lng)
...
def calc_target_bearing(lat: float, lng: float, target_lat: float = TARGET_LAT, target_lng: float = TARGET_LNG) -> float:
    """
    現在地 (lat, lng) から目標座標 (target_lat, target_lng) への
    方位角 [度, 0〜360, 北=0 時計回り] を直接計算して返す。
    """
    phi1 = math.radians(lat)
    phi2 = math.radians(target_lat)
    delta_lambda = math.radians(target_lng - lng)
