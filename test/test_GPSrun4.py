...
概要:
    ...
    GPS の位置変化（前回座標と現在座標の差分）から機体の進行方向（方位角 azimuth）を算出し、
    目標方位（target_bearing）との差分（diff）でモータを制御する。

フロー:
    1. センサ・モータ初期化
    2. GPS Fix 待機
    3. 制御ループ (LOOP_DT 周期)
       │  BNO055 → 加速度・地磁気取得（ログ保持用）
       │  GPS共有変数 → lat/lng 取得
       │  GPS座標変化（移動差分）→ 現在の進行方位 azimuth を計算
       │  angle（目標方位）・distance（距離）計算
...
# --- GPS進行方向計算の閾値 ---
MIN_MOVE_DIST = 0.3         # [m]  GPSノイズによる方位ブレを防ぐため進行方位を更新する最小移動距離
...
# ===========================================================================
# 航法計算 (GPS座標に基づく球面方位角・距離計算)
# ===========================================================================

def calc_distance(lat1: float, lng1: float, lat2: float = TARGET_LAT, lng2: float = TARGET_LNG) -> float:
    """
    地点1 (lat1, lng1) から地点2 (lat2, lng2) までの
    大円距離 [m] を返す (Haversine の公式)。デフォルトの地点2は目標地点。
    """
...
def calc_bearing(lat1: float, lng1: float, lat2: float, lng2: float) -> float:
    """
    地点1 (lat1, lng1) から地点2 (lat2, lng2) への
    方位角 [度, 0〜360, 北=0 時計回り] を計算して返す。
    """
    phi1 = math.radians(lat1)
    phi2 = math.radians(lat2)
    delta_lambda = math.radians(lng2 - lng1)
...
def calc_target_bearing(lat: float, lng: float, target_lat: float = TARGET_LAT, target_lng: float = TARGET_LNG) -> float:
    """現在地 (lat, lng) から目標地点への方位角 [度, 0〜360, 北=0] を返す。"""
    return calc_bearing(lat, lng, target_lat, target_lng)
...
def main():
    ...
    try:
        while True:
            ...
            # ── GPSの位置変化から機体の進行方位 (azimuth) を算出 ──
            if gps_valid:
                if prev_lat == 0.0 and prev_lng == 0.0:
                    prev_lat = lat
                    prev_lng = lng
                else:
                    moved_dist = calc_distance(prev_lat, prev_lng, lat, lng)
                    # 一定距離以上移動した際に進行方向を更新
                    if moved_dist >= MIN_MOVE_DIST:
                        current_heading = calc_bearing(prev_lat, prev_lng, lat, lng)
                        prev_lat = lat
                        prev_lng = lng

            azimuth = current_heading
