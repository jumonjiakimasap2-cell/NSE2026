...
概要:
    ...
    BNO_ORIENTATION により取り付け向き（0°, 90°, 180°, 270°）をハードウェアレベルで補正可能。

フロー:
    1. センサ・モータ初期化 ＆ BNO055 軸再配置 (Axis Remap)
    2. GPS Fix 待機
    3. 制御ループ (LOOP_DT 周期)
...
センサ:
    BNO055  (I2C, NDOF モード, Axis Remap 対応)  ← test_finishv.py / fall.py と同じ
...
def set_bno_orientation(bno: BNO055, orientation: int):
    """
    BNO055 の内部レジスタ (0x41: AXIS_MAP_CONFIG, 0x42: AXIS_MAP_SIGN) を書き換え、
    取り付け向き (1〜4) に応じて X/Y 軸の入れ替えおよび符号反転を行う。
    """
    # Bosch データシート P0〜P3 パターン
    # (AXIS_MAP_CONFIG, AXIS_MAP_SIGN)
    remap_profiles = {
        1: (0x24, 0x00),  # P0: 0°  (正規)
        2: (0x21, 0x04),  # P1: 90° (X/Y入れ替え + X符号反転)
        3: (0x24, 0x06),  # P2: 180°(X, Yともに符号反転)
        4: (0x21, 0x02),  # P3: 270°(X/Y入れ替え + Y符号反転)
    }
    ...
    try:
        # レジスタ変更のため一時的に CONFIGMODE (0x00) に設定
        ...
        # 軸再配置レジスタの書き込み (0x41: CONFIG, 0x42: SIGN)
        if hasattr(bno, 'write_reg'):
...
def calc_azimuth(mag: list) -> float:
    ...
    ※ Axis Remap 適用済みの MagX, MagY を基に計算する。
    """
...
# ===========================================================================
# モータコントローラ (イン・アウト反転修正版)
# ===========================================================================

PIN_PWMA = 13
PIN_AIN1 =  5
PIN_AIN2 =  6
PIN_PWMB = 24  # コメントと一致するよう修正
PIN_BIN1 = 18  # コメントと一致するよう修正
PIN_BIN2 = 23  # コメントと一致するよう修正
PIN_STBY = 11  # STBYピン
...
class MotorController:
    """
    左右モータの独立 PWM 制御。
    gpiozero + LGPIOFactory
    イン (Channel A = 左, Channel B = 右) および アウト (極性) を反転修正済。
    """
    ...
    def __init__(self, speed_fwd: float = SPEED_FWD,
        ...
        # アウト逆転対応: forward と backward のピン指定を反転 (AIN2/BIN2 を forward に割り当て)
        # イン逆転対応: Motor A を左、Motor B を右に割り当て
        self._motor_a = Motor(forward=PIN_AIN2, backward=PIN_AIN1)   # 左モータ
        self._motor_b = Motor(forward=PIN_BIN2, backward=PIN_BIN1)   # 右モータ
