...
def calc_azimuth_with_front(mag: list) -> float:
    """
    ★変更: calc_azimuth() の結果に現在採用中の前方オフセット
    (g_front_offset_deg) を加算し、「機体の物理的な前方」を基準とした
    方位を返す。Phase4 の航法計算・スタック回復の回転角計算は
    全てこちらを使うことで、前方探索で確定した補正が一貫して反映される。

    ★追加: 生の地磁気ベクトルにそのまま atan2 を適用すると、モータへの
    高デューティ通電中に発生する磁気干渉ノイズをそのまま方位計算に
    反映してしまい、機体方位が瞬間的に激しく暴れて誘導走行が発散する
    問題が実機ログで確認された。そのため、まず地磁気ベクトルへ
    EMA (指数移動平均) フィルタ (g_mag_ema, MAG_EMA_ALPHA) を適用して
    ノイズを平滑化した上で calc_azimuth() へ渡す。
    """
    global g_mag_ema
    ...
    return (calc_azimuth(g_mag_ema) + g_front_offset_deg) % 360.0
