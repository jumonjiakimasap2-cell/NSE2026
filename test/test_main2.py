...
フェーズ構成:
    ...
    Phase 3 : キャリブレーション & 誘導走行準備
                ...
                スタック検知の基準加速度を測定する。
                機体自身をモータで8の字に動かし地磁気キャリブレーションを促進する。
...
# --- Phase 3: 8の字自律動作 (地磁気キャリブレーション促進) -------------------
FIG8_ENABLE          = True   # True: モータで自律的に8の字に動く
FIG8_HALF_PERIOD_SEC = 4.0    # [s]  片方向(左 or 右)に旋回し続ける秒数
                              #      この周期で左右を切り替えることで8の字軌道になる
...
def phase3_calibration(bno: BNO055, motor: MotorController, start_time: float) -> float:
    """
    BNO055 キャリブレーション待機。
    地磁気キャリブレーションを促進するため、モータで自律的に8の字軌道を描く
    (FIG8_ENABLE=True の場合)。左右の弱旋回を FIG8_HALF_PERIOD_SEC 秒ごとに
    切り替えることで、円弧を左右交互に描き8の字軌道になる。
    ...
    if FIG8_ENABLE:
        log(f"         モータにより自律8の字動作を実行します (半周期={FIG8_HALF_PERIOD_SEC:.1f} s)")
    else:
        log("         機体を 8 の字に動かして地磁気をキャリブレーションしてください")

    # ヘッダー
    print(f"\n{'経過[s]':>8}  {'Sys':>4}  {'Gyro':>5}  {'Acc':>4}  {'Mag':>4}  {'状態':>12}  {'Motor':>10}")
    print("-" * 62)
    ...
    # ── 8の字動作用の状態 ──
    fig8_dir_left    = True   # True: 左旋回中 / False: 右旋回中
    fig8_last_switch = p3_start
    motor_cmd        = "STOP"

    if FIG8_ENABLE:
        motor.turn_left_weak()
        motor_cmd = "TURN_L_WEAK(FIG8)"
    ...
    while True:
        ...
        # ── 8の字動作: 左右を一定周期で切り替え ──
        if FIG8_ENABLE:
            if now - fig8_last_switch >= FIG8_HALF_PERIOD_SEC:
        ...
        print(f"{elapsed:>8.2f}  {s:>4d}  {gy:>5d}  {ac:>4d}  {mg:>4d}  {status:>12}  {motor_cmd:>10}", flush=True)
    ...
    # 8の字動作を終了し、必ず停止する (静止基準加速度の計測のため)
    motor.stop()
    log("[Phase3] 8の字動作終了 → 停止")
...
def main():
    ...
    try:
        ...
        # Phase 3: キャリブレーション & 誘導準備 (8の字自律動作を含む)
        baseline_horiz_acc = phase3_calibration(bno, motor, start_time)
