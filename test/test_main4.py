...
フェーズ構成:
    ...
    Phase 2 : モータ後退 (30 秒) → 前進 (10 秒)
                サブキャリア脱出 + 初期移動として 30 秒後退した後、
                確実にサブキャリアから離脱・前方へ距離を取るために
                続けて 10 秒前進する。

    Phase 3 : キャリブレーション & 誘導走行準備
                BNO055 の地磁気・加速度・ジャイロキャリブレーション待機。
                【改良】以前は Acc/Gyro が揃うまで完全静止し続けてから
                初めてスピン動作(Mag促進)に入る仕組みだったが、地上機は
                姿勢を変えられないため Acc が上がらずスピンが一度も
                始まらないままタイムアウトする問題があった。
                そこで「静止 CALIB_STILL_SEC 秒 (Acc/Gyro向け) → 左右交互に
                強旋回 FIG8_SPIN_SEC 秒 (Mag向け) → 静止 → …」のサイクルを
                最初から回し続ける方式に変更し、特定軸のキャリブレーションが
                原因で全体が停滞しないようにした。
...
PHASE2_FWD_AFTER_SEC = 10.0 # [s]  ★追加: 後退直後に行う前進秒数
...
def phase2_initial_backward(motor: MotorController, led: LED, start_time: float):
    ...
    # ── ★追加: 後退直後に前進 (サブキャリアから確実に離脱するため) ──
    log(f"[Phase2] 追加前進 {PHASE2_FWD_AFTER_SEC:.0f} 秒")
    motor.forward()
...
def phase3_calibration(bno: BNO055, motor: MotorController, led: LED, start_time: float) -> float:
    ...
    【改良点】
    以前は Acc/Gyro が CALIB_MIN_LEVEL に達するまで完全静止し続け、
    それが達成できて初めてスピン動作(Mag促進)に入る仕組みだったため、
    設置姿勢によっては Acc のキャリブレーションが全く進まず、スピンが
    一度も始まらないまま CALIB_TIMEOUT_SEC でタイムアウトしてしまう
    問題があった (地上機は姿勢を大きく変えられないため)。

    そこで最初から
        静止 CALIB_STILL_SEC 秒 (Acc/Gyro 向け)
        → 左右交互に強旋回 FIG8_SPIN_SEC 秒 (Mag 向け)
        → 静止 CALIB_STILL_SEC 秒
        → …
    というサイクルを回し続ける方式に変更。Acc/Gyro 用の静止機会を
    定期的に必ず確保しつつ、Mag 用のスピン機会も開始直後から与えることで、
    特定軸のキャリブレーションだけが原因で全体が停滞するのを防ぐ。

    さらに CALIB_REPORT_INTERVAL_SEC ごとの進捗まとめログと、
    CALIB_STALL_WARN_SEC 以上レベルが変化しない軸への警告ログを追加。
    ...
    blink_led(led)
    log("─" * 62)
    log(f"[Phase3] キャリブレーション開始 最低レベル={CALIB_MIN_LEVEL}")
    log("[Phase3] 静止(Acc/Gyro) ⇔ スピン(Mag) のサイクルを繰り返します")
    ...
    log(f"[Phase3] STILL: 静止 {CALIB_STILL_SEC:.0f} s (Acc/Gyro 安定化)")
    ...
    while True:
        ...
        # ── 軸別の停滞警告 ──
        for name in AXIS_NAMES:
            if cur[name] < CALIB_MIN_LEVEL and (now - last_improve_time[name]) >= CALIB_STALL_WARN_SEC:
                log(f"[Phase3][WARN] {name} 軸が {CALIB_STALL_WARN_SEC:.0f}s 以上レベル {cur[name]} のまま停滞しています", "WARN")
