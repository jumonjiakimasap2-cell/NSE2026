...
def phase4_guided_run(bno: BNO055, sonar: SonarSensor,
    ...
    last_nav_report_time = p4_start  # ★追加: 目標距離・方位ずれのまとめログ用
    ...
    def forward_with_monitor(duration_sec: float, note_prefix: str) -> bool:
        """
        duration_sec 秒前進しつつ、その間も水平加速度・超音波によるスタック
        判定を継続する。STUCK_COUNT_THRESHOLD 回連続でスタック条件を満たしたら
        その場で打ち切り True (再スタック) を返す。何も起きなければ False。
        こちらもグローバル g_acc / g_sonar_m (メインループが更新) を参照する。
        """
        motor.forward()
