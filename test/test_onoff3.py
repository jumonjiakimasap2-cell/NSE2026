LEDの点灯も無事に確認できたとのこと、良かったです！

物理40番ピン（BCM 21番）のLED制御ロジックと、Linuxのシステムタイマーを利用した「確実にSSHを自動復帰させるバックグラウンド命令（sleep 60 && systemctl start ssh）」を完全に統合した、PC側で実行する完成版プログラムを作成しました。

このコード1本をPCのコマンドプロンプトで実行すれば、ラズパイ側のLEDの状態（常時点灯 ➔ 点滅 ➔ 再点灯 ➔ 消灯）と、SSHのオン・オフ状態がPCの画面ログと完全にリアルタイム同期して進行します。

完全統合版 Pythonコード（PC側で実行）
Python
import time
import socket
import paramiko
